...
{
	private static bool _isMinusNumber;

	internal static string? Calculate(string expression, string separator) 
		=> PostfixEvaluator(InfixToPostfix(expression, separator));

	private static List<Token> InfixToPostfix(string expression, string separator)
	{
		...
		while (i < expression.Length)
		{
			...
			if (char.IsDigit(actualChar) || actualChar == separator[0])
			{
		...
		string MakeNumber()
		{
			...
			while (i < expression.Length && (char.IsDigit(expression[i]) || expression[i] == separator[0]))
			{
				number += expression[i];
				i++;
			}
	...
	private static string PostfixEvaluator(List<Token> tokens)
	{
